<?php 
	...
	if(trim($_GET['tu_khoa'])!=""){ 
		...
		$tv_1=mysqli_query($conn, $tv); // Thay $tv_1=mysql_query($tv);
		$tv_2=mysqli_fetch_array($tv_1); // Thay $tv_2=mysql_fetch_array($tv_1);
		...
		$tv_1=mysqli_query($conn, $tv); // Thay $tv_1=mysql_query($tv);
		echo "<table>";
		while($tv_2=mysqli_fetch_array($tv_1)) // Thay $tv_2=mysql_fetch_array($tv_1);
		{
			echo "<tr>";
				for($i=1;$i<=3;$i++)
				{
